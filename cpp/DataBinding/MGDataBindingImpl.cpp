...
MGDataBindingImpl::MGDataBindingImpl(std::string wishlistId, std::weak_ptr<MGDI> _di): _wishlistId(wishlistId), di(_di) {
    registerBindings();
}

std::set<int> MGDataBindingImpl::applyChangesAndGetDirtyIndices(std::pair<int, int> windowIndexRange) {
    ...
    auto &rt = WishlistJsRuntime::getInstance().getRuntime();
     ...
     if (val.isObject()) {
       ...
       jsi::Array dirtyIndices = f.call(rt, jsi::Value(rt, windowIndexRange.first), jsi::Value(rt, windowIndexRange.second)).asObject(rt).asArray(rt);
         std::set<int> res;
      ...
      binding.setProperty(
          ...
          jsi::Function::createFromHostFunction(
              ...
              [=](jsi::Runtime &rt, const jsi::Value &thisValue, const jsi::Value *args, size_t count) -> jsi::Value {
                  ...
                  std::shared_ptr<MGVSyncRequester> vsr = retainedDI->getVSyncRequester();
                  vsr->requestVSync();
                return jsi::Value::undefined();
              }));
